...
skipall_unless_can_db(driver => 'MySQLCom');

use lib 't/lib';
use DBIx::QuickORM::Test;

my $community_file = __FILE__;
$community_file =~ s/\.t/.sql/;
my $mysql = mysql(load_sql => [quickdb => $community_file]);
    ...
    $schema,
    {
        ...
        tables    => {
            aliases => {
                ...
                columns => {
                    alias_id => {affinity => 'numeric', db_name => 'alias_id', name => 'alias_id', nullable => F(), order => 1, type => \'int'},
            ...
            lights => {
                ...
                columns => {
                    light_id   => {affinity => 'numeric', db_name => 'light_id',   name => 'light_id',   nullable => F(), order => 1, type => \'int'},
